...
some_names <- full_tree$tip.label

modified_names <- data.frame(t(data.frame(str_split(some_names, "s__", n=2))))
colnames(modified_names) <- c("more_tax", "species") 
rownames(modified_names) <- NULL

tree_names <- modified_names %>%
  mutate(fixed_names = gsub("\\:", "_", species))

micro_names[!(micro_names %in% modified_names$species)]

practice_names <- micro_names[grep("Lachnoclostridium", micro_names)]

modified_names$species[grep("Duncaniella", modified_names$species)]
...
saveRDS(big_clean_phyloseq, "C:/Users/hammera/Documents/R_projects/Microbiopics/data/no_alcohol_big_phyloseq.rds")

big_clean_phyloseq <- makePhyloseqFromTreeSummarizedExperiment(tdf, abund_values = "relative_abundance")
```
